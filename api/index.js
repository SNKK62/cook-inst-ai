...
app.get("/", (req, res) => {
  res.json({
    ...
    endpoints: {
      search:
        "GET /search?ingredients=ingredient1,ingredient2 or POST /search with JSON body",
      dbInfo: "/db-info - database structure information",
    },
    examples: {
      getSearch: [
        "http://localhost:3000/search?ingredients=鶏",
        "http://localhost:3000/search?ingredients=豆腐,玉ねぎ",
      ],
      postSearch: {
        method: "POST",
...
app.get("/db-info", async (req, res) => {
  ...
  try {
    ...
    // ingredients配列の長さの統計
    const ingredientStats = await recipes
      .aggregate([
    ...
    res.json({
      ...
      dataStructure: {
        ...
        ingredientsIsArray: firstDoc
          ? Array.isArray(firstDoc.ingredients)
          : false,
